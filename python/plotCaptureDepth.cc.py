...
"""%(os.path.basename(sys.argv[0]), __version__), formatter_class=RawTextHelpFormatter)
    ...
    def outputViolinPlots(self):
        ...
        violin_plot_jpg = "%s.violin_plot.jpg"%self.myArgs.outputRoot
        plt.figure()
        sn.set(style="white")
        ax = sn.violinplot(x="Limit", y="CoRSIV_Ratio", data=self.binarized_coverage_data_frame, width=1.2, linewidth=0.5, cut=0)
        ax.set_ylim(-0.5,1)
        ...
        plt.figure()
        swarm_plot_pdf = "%s.swarm_plot.pdf"%self.myArgs.outputRoot
        swarm_plot_jpg = "%s.swarm_plot.jpg"%self.myArgs.outputRoot
        sn.set(style="white")
        
        box_data = []
        ax = sn.swarmplot(x="Limit", y="CoRSIV_Ratio", data=self.binarized_coverage_data_frame, size=1)
        ax.set_ylim(0,1)
        ax.set_xticklabels(ax.get_xticklabels(), rotation=90)
        plt.savefig(swarm_plot_pdf, bbox_inches='tight', dpi=300)    
        plt.savefig(swarm_plot_jpg, bbox_inches='tight', dpi=300)    
        if (self.DEBUG_PROGRESS):
