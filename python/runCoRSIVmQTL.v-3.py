...
"""%(os.path.basename(sys.argv[0]), __version__), formatter_class=RawTextHelpFormatter)
    ...
    def filterSNVByCorsiv(self, corsiv_line, corsiv_short):
        ...
        temp_SNV_file = "%s%stmp_SNV.cis-1mb.%s.vcf"%(out_corsiv_folder, os.path.sep, corsiv_short)
        bedtools_command = "bedtools window -u -w 1000000 -a %s -b %s > %s"%(self.myArgs.snvFile, one_line_corsiv_bed, temp_SNV_file)

        if (self.DEBUG_PROCESS_ONE_CORSIV):
            sys.stderr.write("out corsiv folder %s tmp snv file %s one-line corsiv bed %s command %s\n"%(out_corsiv_folder, temp_SNV_file, one_line_corsiv_bed, bedtools_command))

        os.system(bedtools_command)
        return temp_SNV_file, out_corsiv_folder
    ...
    # invoke script
    def processOneCoRSIVOneTissue(self, gtex_tissue, filtered_SNV_file, out_corsiv_folder, corsiv_short):
        if (self.DEBUG_PROGRESS):
        ...
        if len(methylation_donors_col_idx)>=20:
            ...
            mQTL_DATA_file = "%s%smethylation_corsiv.%s.tissue.%s.txt"%(out_corsiv_folder, os.path.sep, corsiv_short, gtex_tissue)
            mQTL_DATA_file_writer = open(mQTL_DATA_file, "wt")
            mQTL_DATA_file_writer.write("CoRSIV\t%s\n"%"\t".join(donors_with_methylation_and_SNV_list))


            methylation_tissue_data = self.coverage_df.iloc[corsiv_index, methylation_donors_col_idx]
            if self.myArgs.pearsonCorrelation:
                mQTL_DATA_file_writer.write("%s\t%s\n"%(corsiv_short, "\t".join([str(x) for x in methylation_tissue_data])))
            else:
            ...
            mQTL_DATA_file_writer.close()

            # setup SNV data
            mQTL_SNV_file = "%s%ssnv_corsiv.%s.tissue.%s.txt"%(out_corsiv_folder, os.path.sep, corsiv_short, gtex_tissue)
            mQTL_SNV_file_writer = open(mQTL_SNV_file, "wt")
            mQTL_SNV_file_writer.write("SNV\t%s\n"%"\t".join(donors_with_methylation_and_SNV_list))
            ...
            for snv_line in filtered_SNV_reader:
                ...
                mQTL_SNV_file_writer.write("%s\n"%"\t".join(buffer))

            filtered_SNV_reader.close()
            mQTL_SNV_file_writer.close()

            mQTL_output = "%s%smQTL_corsiv.%s.tissue.%s.xls"%(out_corsiv_folder, os.path.sep, corsiv_short, gtex_tissue)
            log_mQTL_output = "%s%slog.mQTL_corsiv.%s.tissue.%s.txt"%(out_corsiv_folder, os.path.sep, corsiv_short, gtex_tissue)
            eMatrixQTL_command = "driver_eMatrixQTL.cc.R %s %s %s &> %s"%(mQTL_SNV_file, mQTL_DATA_file, mQTL_output, log_mQTL_output)
            os.system(eMatrixQTL_command)
    ...
    def processOneCoRSIV(self, corsiv_line):
        ...
        filtered_SNV_file, out_corsiv_folder = self.filterSNVByCorsiv(corsiv_line, corsiv_short)

        for gtex_tissue in self.tissue_to_donor_hash:
            self.processOneCoRSIVOneTissue(gtex_tissue, filtered_SNV_file, out_corsiv_folder, corsiv_short)
