...
"""%(os.path.basename(sys.argv[0]), __version__), formatter_class=RawTextHelpFormatter)
    ...
    def loadOneTissuemQTL(self, mQTL_csv):
        ...
        for line in mQTL_reader:
            ...
            ff = line.strip().split(',')
            corsiv_index = int(ff[1].replace('"',''))
            chrom_string = ff[2].replace('"','')
            if (self.DEBUG_LOAD_ONE_mQTL):
                sys.stderr.write("ff[2] %s chrom_string %s\n"%(ff[2], chrom_string))

            chrom_number = int(chrom_string.replace("chr",""))
            snp_string = ff[6].replace('"','')
            if (self.DEBUG_LOAD_ONE_mQTL):
                sys.stderr.write("ff[6] %s snp_string %s \n"%(ff[6], snp_string))

            gg = snp_string.split('_')
            snp_pos = int(gg[1])
            beta = float(ff[len(ff)-2])
            if (beta>0):
                beta_sign = 1
            else:
                beta_sign = -1
...
#            output_report_writer.write("%s\n"%heatmap_key)
    ...
    def outputCombinedmQTLProfiles2(self):
        ...
        for tissue in self.mQTL_SNP_CoRSIV_hash:
            ...
            if (self.DEBUG_OUTPUT_COMBINED_MQTL):
                sys.stderr.write("collecting keys from tissue %s hash_size %s\n"%(tissue, len(tissue_hash_snp)))
            ...
            if (self.DEBUG_OUTPUT_BLOCK):
                sys.stderr.write("collecting blocks from tissue %s items %s\n"%(tissue, len(tissue_hash_block)))
        ...
        output_report_abs = "%s.snp_report.genomic_coord_corsiv_index.abs.xls"%self.myArgs.outputRoot
        output_report_writer = open(output_report, "wt")
        output_report_abs_writer = open(output_report_abs, "wt")
        output_report_writer.write("SNP\t%s\n"%"\t".join(self.mQTL_file_basename_list))
        output_report_abs_writer.write("SNP\t%s\n"%"\t".join(self.mQTL_file_basename_list))
        ...
        for idx in range(len(self.snp_corsiv_key_list)):
            ...
            heatmap_key = "chr%s_%s_corsiv_%s"%(tuple_key[0], tuple_key[1], tuple_key[2])
            if (self.DEBUG_OUTPUT_COMBINED_MQTL):
                sys.stderr.write("idx %s tuple_key %s snp_key %s heatmap_key %s\n"%(idx, tuple_key, snp_key, heatmap_key))
            ...
            for tissue in self.mQTL_file_basename_list:
                ...
                if (self.DEBUG_OUTPUT_COMBINED_MQTL):
                    sys.stderr.write("snp_buffer %s\n"%";".join([str(x) for x in snp_buffer]))

            output_report_writer.write("%s\t%s\n"%(heatmap_key, "\t".join([str(x) for x in snp_buffer])))
            output_report_abs_writer.write("%s\t%s\n"%(heatmap_key, "\t".join([str(abs(x)) for x in snp_buffer])))

        output_report_writer.close()
        output_report_abs_writer.close()

        # block level report
        self.block_corsiv_key_list = list(self.block_corsiv_key_set)
        self.block_corsiv_key_list.sort()

        # generate two SNV-level recurrence reports: one with direction -1/0/1 and the other one with 0/1 (eg present absent)
        output_block_report = "%s.block_report.genomic_coord_corsiv_index.xls"%self.myArgs.outputRoot
        output_block_report_abs = "%s.block_report.genomic_coord_corsiv_index.abs.xls"%self.myArgs.outputRoot
        output_block_report_writer = open(output_block_report, "wt")
        output_block_report_abs_writer = open(output_block_report_abs, "wt")
        output_block_report_writer.write("Block\t%s\n"%"\t".join(self.mQTL_file_basename_list))
        output_block_report_abs_writer.write("Block\t%s\n"%"\t".join(self.mQTL_file_basename_list))

        if (self.DEBUG_OUTPUT_BLOCK):
            sys.stderr.write("block_corsiv_key_list %s items\n"%len(self.block_corsiv_key_list))
        ...
        for idx in range(len(self.block_corsiv_key_list)):
            ...
            heatmap_key = "chr%s_%s_corsiv_%s"%(tuple_key[0], tuple_key[1], tuple_key[2])
            block_buffer = [] # print heatmap_key explicitly

            if (self.DEBUG_OUTPUT_BLOCK):
                sys.stderr.write("block: idx %s tuple_key %s snp_key %s heatmap_key %s\n"%(idx, tuple_key, block_key, heatmap_key))
            ...
            for tissue in self.mQTL_file_basename_list:
                ...
                if (self.DEBUG_OUTPUT_BLOCK):
                    sys.stderr.write("block_buffer %s\n"%";".join([str(x) for x in block_buffer]))

            output_block_report_writer.write("%s\t%s\n"%(heatmap_key, "\t".join([str(x) for x in block_buffer])))
            output_block_report_abs_writer.write("%s\t%s\n"%(heatmap_key, "\t".join([str(abs(x)) for x in block_buffer])))

        output_block_report_writer.close()
        output_block_report_abs_writer.close()
