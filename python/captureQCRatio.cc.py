...
"""%(os.path.basename(sys.argv[0]), __version__), formatter_class=RawTextHelpFormatter)
        ...
        parser.add_argument('-r','--ratio',             help='minimum read ratio (0,1] overlap with capture target',  required=True)
    ...
    def determineCaptureOverlap(self):
        ...
        for full_bed in self.full_bed_list:
            ...
            bedtool_intersect_ratio_command = "bedtools intersect -u -f %s -a %s -b %s | pigz -p 2 -c > %s"%(self.myArgs.ratio, full_bed, self.myArgs.captureTarget, overlap_bed_file)
            
            if (self.DEBUG_OVERLAP):
                sys.stderr.write("bedtools intersect command %s\n"%bedtool_intersect_ratio_command)
                
            os.system(bedtool_intersect_ratio_command)
