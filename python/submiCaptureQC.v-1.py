...
TODO: play with tmp dir location; for now assume ${TMPDIR}
        ...
        parser.add_argument('-r','--radius',            help='radius around capture targets (in basepairs)',        default="0", required=False)
    ...
    def generateClusterJobPreamble(self):
        ...
        if (self.myArgs.clusterType == "PBS" ):
            self.clusterJobFile = "%s/captureQC.%s.%s.pbs"%(self.mappingFolder, self.dName, os.getpid())
            ...
            self.clusterJobFileWriter.write("#PBS -o "+self.mappingFolder+"/"+self.dName+".captureQC."+str(os.getpid())+".o\n")
            self.clusterJobFileWriter.write("#PBS -e "+self.mappingFolder+"/"+self.dName+".captureQC."+str(os.getpid())+".e\n")
            
        elif(self.myArgs.clusterType == "SLURM"):
            self.clusterJobFile = "%s/captureQC.%s.%s.slurm"%(self.mappingFolder, self.dName, os.getpid())
            self.clusterJobFile = self.mappingFolder +"/captureQC."+self.dName+"."+os.getpid()+".slurm"
            self.clusterJobFileWriter = open(self.clusterJobFile, "w")
            ...
            self.clusterJobFileWriter.write("#SBATCH -o "+self.mappingFolder +"/" +self.dName + ".captureQC."+str(os.getpid())+".%N.%J.o\n")
            self.clusterJobFileWriter.write("#SBATCH -e "+self.mappingFolder +"/" +self.dName + ".captureQC."+str(os.getpid())+".%N.%J.e\n")
            self.clusterJobFileWriter.write("#SBATCH -J captureQC"+self.dName+"."+ str(os.getpid())+"\n")
    ...
    def generateJobCommands(self):
        ...
        captureQC_command = "captureQC.cc.py -b \"%s\" -c %s -r %s -e %s -o %s -k "%(os.path.abspath(self.myArgs.bamFilePattern), os.path.abspath(self.myArgs.captureTarget), self.myArgs.radius, self.myArgs.experiment, os.path.basename(self.myArgs.outputRoot))
