...
class AssessCaptureCORSiV:
    ...
    DEBUG_HEATMAP_2LEVEL_COLOR      = False
    DEBUG_HEATMAP_2LEVEL_COLOR      = False
...
"""%(os.path.basename(sys.argv[0]), __version__), formatter_class=RawTextHelpFormatter)
        ...
        parser.add_argument('-c','--tissueColors',              help='tissue color map',       required=True)
    ...
    def loadTissueColors(self):
        if (self.DEBUG_PROGRESS):
            sys.stderr.write("[%s] START AssessCaptureCORSiV::loadTissueColors\n"%SimpleTime.now())
        
        self.tissue_color_hash = {}
        
        tissue_color_reader = open(self.myArgs.tissueColors, "rt")
        line_idx = -1
        for line in tissue_color_reader:
            line_idx +=1
            if (line_idx==0):
                continue
            ff = line.strip().split('\t')
            gtex_tissue_id = ff[0]
            color_rgb = ff[1]
            self.tissue_color_hash[gtex_tissue_id]=color_rgb
            if (self.DEBUG_HEATMAP_2LEVEL_COLOR):
                sys.stderr.write("Loading tissue %s color %s\n"%(gtex_tissue_id, color_rgb))
        
        tissue_color_reader.close()
        
        if (self.DEBUG_PROGRESS):
            sys.stderr.write("[%s] STOP AssessCaptureCORSiV::loadTissueColors\n"%SimpleTime.now())
    ...
    def generateMultiTissueHeatmaps(self):    
        ...
        # Setting up the donors colors
#        donor_colors_cmap = cm.get_cmap('viridis', 100)
        donor_colors_cmap = SimpleData.generate_rich_colormap(60)
        
        # Setting up the tissue colors
        # tissue_colors_pallette = ['b', 'g', 'r', 'c', 'm', 'y']
#        tissue_colors_pallette = [donor_colors_cmap(0), donor_colors_cmap(0.2), donor_colors_cmap(0.4), donor_colors_cmap(0.6), donor_colors_cmap(0.8), donor_colors_cmap(1.0)]
#        tissue_idx = -1
#        for gtex_tissue_id in self.tissue_to_donor_hash:
#            tissue_idx += 1
#            tissue_color_hash[gtex_tissue_id] = tissue_colors_pallette[tissue_idx]
        
        self.loadTissueColors()

        columns = list(self.coverage_df.columns)         
        
        for min_tissue_coverage in range(4, self.max_coverage+1):
            if (self.DEBUG_HEATMAP_2LEVEL_COLOR):
                sys.stderr.write("Generating heatmap for donors with at least %s tissues\n"%min_tissue_coverage)
            
            

            # first identify how many donors pass the min tissue criteria
            passing_donors = 0
            ...
            if (self.DEBUG_HEATMAP_2LEVEL_COLOR):
                sys.stderr.write("Found %s donors with at least %s tissues\n"%(passing_donors, min_tissue_coverage))
            
            tissue_donor_color_list = []
            tissue_celltype_color_list = []
            column_ids_list = []
            donor_idx = 0 
            ...
            for gtex_donor_id in self.multi_tissue_hash:
                ...
                if (donor_tissue_count>=min_tissue_coverage):
                    donor_idx += 1
                    donor_ratio = float(donor_idx) / float(passing_donors)
                    donor_color = donor_colors_cmap(donor_ratio)
                    
                    column_ids_set = self.multi_tissue_hash[gtex_donor_id]
                    
                    for column_idx in column_ids_set:
                        tissue_donor_color_list.append(donor_color)
                        column_name = columns[column_idx]
                        ff = column_name.split()
                        seq_file_id = ff[0]
                        gtex_tissue_id = self.tissue_hash[seq_file_id].gtex_tissue_id
                        gtex_tissue_color = self.tissue_color_hash[gtex_tissue_id]
                        tissue_celltype_color_list.append(gtex_tissue_color)
                        
                        column_ids_list.append(column_idx)
                    
                    if (self.DEBUG_MULTI_TISSUE_HEATMAPS):
                        sys.stderr.write("Donor %s with %s tissues >= %s --> tally %s tissues\n"%(gtex_donor_id, donor_tissue_count, min_tissue_coverage, len(column_ids_list)))
                        
                        
                                        
            self.min_coverage_to_tissue_hash[min_tissue_coverage] = column_ids_list
            # TODO: here select samples and perfom heatmap
            slice_df = self.coverage_df.iloc[:,column_ids_list]
            ...
            plt.figure()
            sn.set()
            sn.clustermap(slice_df,  cmap='bwr', col_colors=[tissue_celltype_color_list, tissue_donor_color_list], row_cluster=True, col_cluster=True, metric="euclidean", linewidths=0.0, rasterized=True, figsize=(int(float(slice_df.shape[1])*0.3),30))
