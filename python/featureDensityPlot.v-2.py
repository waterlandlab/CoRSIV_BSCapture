...
"""%(os.path.basename(sys.argv[0]), __version__), formatter_class=RawTextHelpFormatter)
    ...
    def runWindowRadiusCoverage(self, window_radius_file, bed_file, window_hash):
        ...
        eRNA_command = "eRNAScore.cc.py -r %s -e %s -x %s -p 2 -C %s -o %s &> %s_log.eRNA.%s.txt"%(self.myArgs.genomicFeaturesBed,
                window_radius_file, tmp_folder, self.myArgs.chromosomeMap, eRNA_scores, self.myArgs.outputRoot, os.path.basename(window_radius_file))
    ...
    def outputCombinedFeatureDensityPlot(self):
        ...
        support_data_writer.write("Distance from BED center\t%s\t%s\n"%(os.path.basename(self.myArgs.bedFile1), os.path.basename(self.myArgs.bedFile2)))
        ...
        ax1.bar(x_axis, self.density_around_bed_file1.coverage_avg, width=0.8)
        ...
        ax2.bar(x_axis, self.density_around_bed_file2.coverage_avg, width=0.8)
