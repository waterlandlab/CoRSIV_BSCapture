...
"""%(os.path.basename(sys.argv[0]), __version__), formatter_class=RawTextHelpFormatter)
    ...
    def loadCoRSIVDef(self):
        ...
        for line in corsiv_reader:
            ...
            corsiv_info = CStruct(corsiv_short = corsiv_short, chrom = chrom, chrom_start = chrom_start, chrom_stop = chrom_stop, gene=gene)
    ...
    def processOneCoRSIV(self, one_mQTL_file):
        ...
        for line in corsiv_mQTL_reader:
            ...
            if (abs(snv_corsiv_distance)>1000000):
                continue
            
            buffer  = [ff[0], corsiv_short, snv_corsiv_distance, ff[2], ff[3], ff[4], corsiv_info.gene ]
    ...
    def processAllCoRSIVs(self):
        ...
        header = ["SNP", "CoRSIV", "Distance", "beta", "tstat", "pvalue", "gene"]
        self.mQTL_annotated_report_writer.write("%s\n"%"\t".join(header))
