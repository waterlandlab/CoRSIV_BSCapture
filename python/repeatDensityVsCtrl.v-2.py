...
"""%(os.path.basename(sys.argv[0]), __version__), formatter_class=RawTextHelpFormatter)
        ...
        parser.add_argument('-k','--keepTempFiles',         help='keep temporary files (note: that's a lot of files)',   required=False, action="store_true")
    ...
    def reportOddsRatioCumulative(self):
        ...
        cumulative_report_linear = "%s.0k_%sk_cumulative_odds_ratios.xls"%(self.myArgs.outputRoot, int(self.genomic_window/1000))
        cumulative_report_linear_writer = open(cumulative_report_linear, "wt")
        cumulative_report_log2 = "%s.0k_%sk_cumulative_log2_odds_ratios.xls"%(self.myArgs.outputRoot, int(self.genomic_window/1000))
        ...
        for bed_file_base in sorted_bed_file_bases:
            ...
            individual_bed_overlap_report = "%s.invidual_overlap_wrt_control.%s.xls"%(self.myArgs.outputRoot, bed_file_base)
    ...
    def performComparisons(self):
        if (self.DEBUG_PROGRESS):
            sys.stderr.write("[%s] START FeatureRepeatOverlap::performComparisons\n"%SimpleTime.now())
        ...
        for line in comparison_reader:
            ...
            # also produce a per-file overlap with log2 odds ratio and p-value
            one_comparison_report = "%s.comparison.%s.xls"%(self.myArgs.outputRoot, comparison_label)
        ...
        if (self.DEBUG_PROGRESS):
            sys.stderr.write("[%s] STOP FeatureRepeatOverlap::performComparisons" %SimpleTime.now())
