...
"""%(os.path.basename(sys.argv[0]), __version__), formatter_class=RawTextHelpFormatter)
        ...
        parser.add_argument('-R','--minimumAbsPCC',       help='minimum absolute value of PCC to consider',    required=True)
    ...
    def summarizeCounts(self):
        ...
        min_abs_pcc = abs(float(self.myArgs.minimumAbsPCC))
        ...
        for corsiv_idx in range(len(self.corr_index)):
            ...
            for col_idx in range(len(self.corr_columns)):
                ...
                if (corr_value>=-1 and corr_value<-min_abs_pcc):
                    corsiv_corr_count["ALL_negative_corr"] +=1
                elif (corr_value>min_abs_pcc and corr_value<=1):
    ...
    def doTargetSurrogateReport(self):
        ...
        for corsiv_idx in range(len(self.corr_index)):
            ...
            for rna_tissue in self.rna_labels:
                ...
                if (target_meth_rna_corr>0) and (target_meth_rna_corr<=1):
                    ...
                    for surrogate_tissue in self.surrogate_tissues:
                        ...
                        if (surrogate_meth_target_rna_corr>0) and (surrogate_meth_target_rna_corr<=1):
                        ...
                        elif (surrogate_meth_target_rna_corr>=-1) and (surrogate_meth_target_rna_corr<-min_abs_pcc):
                ...
                # check target rna/meth correlation
                elif (target_meth_rna_corr>=-1) and (target_meth_rna_corr<0):
                    ...
                    for surrogate_tissue in self.surrogate_tissues:
                        ...
                        elif (surrogate_meth_target_rna_corr>=-1) and (surrogate_meth_target_rna_corr<0):
