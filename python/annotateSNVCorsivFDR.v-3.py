...
class Annotate_CoRSIV_mQTL:
    ...
    DEBUG_PROCESS_ONE_MQTL         = False
    DEBUG_SNV_CORSIV_DISTANCE      = True
...
apply Simes correction to a set of rank-methylation EMatrixQTL results
"""%(os.path.basename(sys.argv[0]), __version__), formatter_class=RawTextHelpFormatter)
    ...
    def processOneCoRSIV(self):
        ...
        tmp_simes_header = ["Link_index", "snv", "corsiv_short", "distance", "pValue"]
        ...
        for line in corsiv_mQTL_reader:
            ...
            if (snv_corsiv_distance<=1000000):
                self.snv_distance_hash[snv_info]=snv_corsiv_distance
                current_beta = float(ff[2])
                current_pvalue = float(ff[4])

                simes_input_buffer = ["link_%s"%line_idx, snv_info, corsiv_short, snv_corsiv_distance, current_pvalue]

                tmp_simes_input_writer.write("%s\n"%",".join([str(x) for x in simes_input_buffer]))

                if (min_pvalue>current_pvalue): # this ensures the first line gets incorporated
        ...
        min_simes_qvalue = 2 # this ensures that the first line in the output file seeds the best SNP
        min_simes_distance = None
        min_simes_snp = ""
        line_idx = -1
        for line in simes_output_reader:
            ...
            if (q_value<min_simes_qvalue):
                min_simes_snp = ff[2]
                min_simes_distance = ff[4]
                min_simes_qvalue = q_value
                min_simes_pvalue = ff[5]
        ...
        annotated_corsiv_significant_writer.write("CoRSIV_short,snp,distance,min_pValue,simes_qvalue\n")
        significant_simes_data = [corsiv_short, min_simes_snp, min_simes_distance, min_simes_pvalue, min_simes_qvalue]
