...
class CaptureCORSiV_vs_GeneExpression:
    ...
    DEBUG_PCC_RNA_METH_PROGRESS     = False
    DEBUG_REPORT_GENE_NOT_FOUND     = False
    DEBUG_REPORT_EXPRESSED_GENES    = False
    DEBUG_RNA_METH_SCATTERPLOTS_SETUP       = True
    DEBUG_RNA_METH_SCATTERPLOTS_PREP        = True
    DEBUG_RNA_METH_SCATTERPLOTS_VISUALIZE   = True
...
"""%(os.path.basename(sys.argv[0]), __version__), formatter_class=RawTextHelpFormatter)
        ...
        parser.add_argument('-T','--TPMcutoff',                 help='TPM cutoff for median gene expression',       default='0.5')
        parser.add_argument('-l','--corsivGenePlotList',        help='file containing list of corsiv/gene for which to generate scatterplots',  required = False)
    ...
    def setupRNAMethylationScatterplots(self):
        if (self.DEBUG_PROGRESS):
            sys.stderr.write("[%s] START CaptureCORSiV_vs_GeneExpression::setupRNAMethylationScatterplots\n"%SimpleTime.now())

        self.scatterplot_hash = {}
        
        corsiv_tissue_scatterplot_list_reader = open(self.myArgs.corsivGenePlotList, "rt")
        
        for line in corsiv_tissue_scatterplot_list_reader:
            ff = line.strip().split('\t')
            target_tissue = ff[1]
            final_corsiv_id = ff[0]
            y_lim = int(ff[2])
            corsiv_target_key = "%s_targetTissue_%s"%(final_corsiv_id, target_tissue)
            corsiv_target_info = CStruct(final_corsiv = final_corsiv_id, target_tissue = target_tissue, target_corr_4uple=False, blood_corr_4uple=False, skin_corr_4uple=False, y_lim = y_lim)
            self.scatterplot_hash[corsiv_target_key]=corsiv_target_info
            if (self.DEBUG_RNA_METH_SCATTERPLOTS_SETUP):
                    sys.stderr.write("Setup RNAxMETH scatterplot for corsiv %s target %s key %s\n"%(final_corsiv_id, target_tissue, corsiv_target_key))
        
        corsiv_tissue_scatterplot_list_reader.close()
                
        if (self.DEBUG_PROGRESS):
            sys.stderr.write("[%s] STOP CaptureCORSiV_vs_GeneExpression::setupRNAMethylationScatterplots\n"%SimpleTime.now())
    ...
    def computeMethylationGeneExpCorrelation(self):
        ...
        if (self.myArgs.corsivGenePlotList):
            self.setupRNAMethylationScatterplots()
        ...
        for meth_tissue_idx in range(len(meth_tissue_list)):
            ...
            for rna_tissue_idx in range(len(rna_tissue_list)):
                ...
                if not (rna_tissue_id) in meth_tissue_list:
                    sys.stderr.write("EEk: rna tissue %s not found in meth tissue list %s \n"%(rna_tissue_id, "; ".join(meth_tissue_list)))
                    sys.exit(4)
                    
                    
                # check whether rna tissue == meth tissue or meth tissue = skin or meth tisue = blood
                if (meth_tissue_id == rna_tissue_id) or (meth_tissue_id.find("Blood")>=0) or (meth_tissue_id.find("Skin")>=0):
                    if (self.DEBUG_PCC_RNA_METH):
                        sys.stderr.write("Passing_pair meth [%s] %s vs rna [%s] %s\n"%(meth_tissue_idx, meth_tissue_id, rna_tissue_idx, rna_tissue_id))
                else:   
                    if (self.DEBUG_PCC_RNA_METH):
                        sys.stderr.write("Failing_pair meth [%s] %s vs rna [%s] %s\n"%(meth_tissue_idx, meth_tissue_id, rna_tissue_idx, rna_tissue_id))
                    continue
                ...
                if (len(common_meth_rna_donors)<self.min_donors_correlation):
                ...
                for capture_corsiv in self.capture_corsiv_to_short_corsiv_hash:
                    ...
                    for gene_symbol in gene_list:
                        ...
                        if (len(meth_value_meth_col_idx)<self.min_donors_correlation):
                            self.meth_rna_pair_corr_hash[tissue_pair][final_corsiv_id]=(-2, 1)
                            if (self.DEBUG_PCA_RNA_METH_CORSIV):
                        ...
                        # select appropriate arrays
                            ...
                            rna_in_tissue_and_donors_median = np.median(rna_values_array)
                            if (rna_in_tissue_and_donors_median>=self.tpm_cutoff):
                                ...
                                # if (pcc_pvalue<0.05):
                                #    # if significant > yes
                                self.meth_rna_pair_corr_hash[tissue_pair][final_corsiv_id]=(pcc_value, pcc_pvalue)
                                #else:
                                #    # if not significant 0
                                #    self.meth_rna_pair_corr_hash[tissue_pair][final_corsiv_id]=0
                                
                                # collect info if needed for gene expression/DNA methylation scatterplots
                                if (self.myArgs.corsivGenePlotList):
                                    corsiv_target_key = "%s_targetTissue_%s"%(final_corsiv_id, rna_tissue_id)
                                    if (corsiv_target_key in self.scatterplot_hash):
                                        if (self.DEBUG_RNA_METH_SCATTERPLOTS_PREP):
                                            sys.stderr.write("corsiv_x_meth_rna_tissue_key_found: %s\n"%corsiv_target_key)
                                            
                                        corsiv_target_info = self.scatterplot_hash[corsiv_target_key]    
                                        # now check if methylation tissue is target, blood, or skin
                                        if (meth_tissue_id==rna_tissue_id):
                                            if (self.DEBUG_RNA_METH_SCATTERPLOTS_PREP):
                                                sys.stderr.write("corsiv_x_meth_rna_tissue_key_found ON_TARGET: %s %s \n"%(corsiv_target_key, meth_tissue_id))
                                            corsiv_target_info.target_corr_4uple = (np.copy(meth_values_array), np.copy(rna_values_array), pcc_value, pcc_pvalue)
                                        elif (meth_tissue_id=="Whole_Blood"):
                                            if (self.DEBUG_RNA_METH_SCATTERPLOTS_PREP):
                                                sys.stderr.write("corsiv_x_meth_rna_tissue_key_found ON_Blood: %s %s \n"%(corsiv_target_key, meth_tissue_id))
                                            corsiv_target_info.blood_corr_4uple = (np.copy(meth_values_array), np.copy(rna_values_array), pcc_value, pcc_pvalue)
                                        elif (meth_tissue_id=="Skin"):
                                            if (self.DEBUG_RNA_METH_SCATTERPLOTS_PREP):
                                                sys.stderr.write("corsiv_x_meth_rna_tissue_key_found ON_Skin: %s %s \n"%(corsiv_target_key, meth_tissue_id))
                                            corsiv_target_info.skin_corr_4uple = (np.copy(meth_values_array), np.copy(rna_values_array), pcc_value, pcc_pvalue)
                                        else:
                                            if (self.DEBUG_RNA_METH_SCATTERPLOTS_PREP):
                                                sys.stderr.write("corsiv_x_meth_rna_tissue_key_found ON_Other_Internal: %s %s \n"%(corsiv_target_key, meth_tissue_id))
                                    else:
                                        if (self.DEBUG_RNA_METH_SCATTERPLOTS_PREP):
                                            sys.stderr.write("corsiv_x_meth_rna_tissue_key_not_found: %s\n"%corsiv_target_key)
                                        
                            else:
                                self.meth_rna_pair_corr_hash[tissue_pair][final_corsiv_id]=(-4, 1)
    ...
    def reportAndVisualizeMethRNACorrelation(self):
        ...
        meth_rna_corr_file_writer.write("%s\t%s\n"%(corsiv_header,'\t'.join(["%s\t%s pvalue"%(x,x) for x in meth_rna_pair_list])))
        ...
        for final_corsiv in final_corsiv_list:
            ...
            for meth_rna_pair in meth_rna_pair_list:
                ...
                pcc, pvalue = final_corsiv_hash[final_corsiv]
                buffer.append(pcc)
...
#        plt.savefig(jpg_heatmap_file, dpi=150)
        ...
        if (self.myArgs.corsivGenePlotList):
            self.generateCoRSIVRNAMethylationScatterplots()
        
        if (self.DEBUG_PROGRESS):
            sys.stderr.write("[%s] STOP CaptureCORSiV_vs_GeneExpression::report_and_visualize_rna_meth_corr\n"%SimpleTime.now())
    
    def generateCoRSIVRNAMethylationScatterplots(self):
        if (self.DEBUG_PROGRESS):
            sys.stderr.write("[%s] START CaptureCORSiV_vs_GeneExpression::generateCoRSIVRNAMethylationScatterplots\n"%SimpleTime.now())
        rna_meth_scatterplots_folder = "%s.RNA_DNA_SCATTERPLOT.folder"%self.myArgs.outputRoot
        mkdir_command = "mkdir -p %s"%rna_meth_scatterplots_folder
        if (self.DEBUG_RNA_METH_SCATTERPLOTS_VISUALIZE):
            sys.stderr.write("mkdir command for scatterplots folder %s\n"%mkdir_command)
        os.system(mkdir_command)
        
        for corsiv_target_key in self.scatterplot_hash:
            corsiv_rna_meth_scatterplot_pdf = "%s.RNA_DNA_SCATTERPLOT.folder/%s.%s.pdf"%(self.myArgs.outputRoot, os.path.basename(self.myArgs.outputRoot), corsiv_target_key)
            
            if (self.DEBUG_RNA_METH_SCATTERPLOTS_VISUALIZE):
                sys.stderr.write("Scatterplot file %s\n"%corsiv_rna_meth_scatterplot_pdf)
           
            corsiv_scatterplot_info = self.scatterplot_hash[corsiv_target_key]
            
            plt.figure()
            fig, ax = plt.subplots(1, 3, figsize=(20,6))
           
            # plot correlation on target
            if (corsiv_scatterplot_info.target_corr_4uple):
                meth_values_array, rna_values_array, pcc, pcc_pvalue = corsiv_scatterplot_info.target_corr_4uple
                ax[0].set_xlabel("DNA Methylation %s"%corsiv_scatterplot_info.target_tissue) 
                ax[0].set_ylabel("Gene Expression %s (TPM)"%corsiv_scatterplot_info.target_tissue)
                
                ax[0].scatter(meth_values_array, rna_values_array, color='b')
                
                b, m = polyfit(np.asfarray(meth_values_array), np.asfarray(rna_values_array), 1)
                            
                ax[0].plot(meth_values_array, b+m*meth_values_array, '-', color='k')
                ax[0].set_xlim(0,1)
                if (corsiv_scatterplot_info.y_lim>0):
                    ax[0].set_ylim(0,corsiv_scatterplot_info.y_lim)
                    
                ax[0].set_title("PCC R=%.4f N=%s p-value=%.3e"%(pcc, len(meth_values_array), pcc_pvalue))
             
            # plot correlation on blood
            
            if (corsiv_scatterplot_info.blood_corr_4uple):
                meth_values_array, rna_values_array, pcc, pcc_pvalue = corsiv_scatterplot_info.blood_corr_4uple
                ax[1].set_xlabel("DNA Methylation Whole Blood")
                ax[1].set_ylabel("Gene Expression %s (TPM)"%corsiv_scatterplot_info.target_tissue)
               
                ax[1].scatter(meth_values_array, rna_values_array, color='b')
                
                b, m = polyfit(np.asfarray(meth_values_array), np.asfarray(rna_values_array), 1)
                            
                ax[1].plot(meth_values_array, b+m*meth_values_array, '-', color='k')
                ax[1].set_xlim(0,1)
                if (corsiv_scatterplot_info.y_lim>0):
                    ax[1].set_ylim(0,corsiv_scatterplot_info.y_lim)
                    
                ax[1].set_title("PCC R=%.4f N=%s p-value=%.3e"%(pcc, len(meth_values_array), pcc_pvalue))
                        
            # plot correlation on skin
            
            if (corsiv_scatterplot_info.skin_corr_4uple):
                meth_values_array, rna_values_array, pcc, pcc_pvalue = corsiv_scatterplot_info.skin_corr_4uple
            
                ax[2].set_xlabel("DNA Methylation Skin")
                ax[2].set_ylabel("Gene Expression %s (TPM)"%corsiv_scatterplot_info.target_tissue)
                
                ax[2].scatter(meth_values_array, rna_values_array, color='b')
                
                b, m = polyfit(np.asfarray(meth_values_array), np.asfarray(rna_values_array), 1)
                            
                ax[2].plot(meth_values_array, b+m*meth_values_array, '-', color='k')
                ax[2].set_xlim(0,1)
                if (corsiv_scatterplot_info.y_lim>0):
                    ax[2].set_ylim(0,corsiv_scatterplot_info.y_lim)
                    
                ax[2].set_title("PCC R=%.4f N=%s p-value=%.3e"%(pcc, len(meth_values_array), pcc_pvalue))
                        
            plt.savefig(corsiv_rna_meth_scatterplot_pdf, dpi=300)
           
        if (self.DEBUG_PROGRESS):
            sys.stderr.write("[%s] STOP CaptureCORSiV_vs_GeneExpression::generateCoRSIVRNAMethylationScatterplots\n"%SimpleTime.now())

    def reportExpressedGenes(self):
        if (self.DEBUG_PROGRESS):
            sys.stderr.write("[%s] START CaptureCORSiV_vs_GeneExpression::reportExpressedGenes\n"%SimpleTime.now())
        
        # setup tissue hash
        # setup reports: full gene/tissue; small tissue: expressed genes
        # for each gene
        #    setup buffer list
        #    for each RNA tissue
        #       collect samples from one rna tissue
        #       extract full set of samples
        #       compute median expression
        #       if median>=cutoff: add to expressed hash
        #       add anyway to buffer
        #  plot buffer
        #  small report: output hash of expressed genes
        
        gene_tissue_median_expression_report = "%s.gene_tissue.xls"%self.myArgs.outputRoot
        gene_tissue_median_expression_report_writer = open(gene_tissue_median_expression_report, "wt")
        
        gene_tissue_present_report = "%s.gene_tissue_present.xls"%self.myArgs.outputRoot
        gene_tissue_present_report_writer = open(gene_tissue_present_report, "wt")
        
        
        rna_tissue_list = sorted(list(self.gene_tissue_hash.keys()))
        gene_tissue_median_expression_report_writer.write("Gene\t%s\n"%"\t".join(rna_tissue_list)) 
        gene_tissue_present_report_writer.write("Gene\t%s\n"%"\t".join(rna_tissue_list))
        
        self.expressed_genes_hash = {}
        for rna_tissue_id in rna_tissue_list:
            self.expressed_genes_hash[rna_tissue_id]=0
        
        for gene_idx in range(len(self.gtex_gene_index)):
            gene_symbol = self.gtex_gene_expression_df.iat[gene_idx, 0]
                        
            buffer = [gene_symbol]
            buffer_present = [gene_symbol]
            
            for rna_tissue_id in rna_tissue_list:
                if (self.DEBUG_REPORT_EXPRESSED_GENES):
                    sys.stderr.write("ReportGex [%s] %s x %s\n"%(gene_idx, gene_symbol, rna_tissue_id))
                    
                rna_tissue_donors = self.rna_tissue_to_donor_hash[rna_tissue_id].keys()
                rna_col_idx_list = []
                for rna_donor in rna_tissue_donors:
                    rna_col_idx = self.rna_tissue_to_donor_hash[rna_tissue_id][rna_donor]
                    rna_col_idx_list.append(rna_col_idx)
                    
                rna_in_tissue_array = self.gtex_gene_expression_df.iloc[gene_idx, rna_col_idx_list]
                rna_in_tissue_median = np.median(rna_in_tissue_array)
                buffer.append(rna_in_tissue_median)
                
                if (rna_in_tissue_median>=self.tpm_cutoff):
                    self.expressed_genes_hash[rna_tissue_id] += 1
                    if (self.DEBUG_REPORT_EXPRESSED_GENES):
                        sys.stderr.write("YayReportGex [%s] %s x %s: %s donors median %s\n"%(gene_idx, gene_symbol, rna_tissue_id,  len(rna_in_tissue_array), rna_in_tissue_median))
                    buffer_present.append(1)
                else:
                    buffer_present.append(0)
                    if (self.DEBUG_REPORT_EXPRESSED_GENES):
                        sys.stderr.write("NyayReportGex [%s] %s x %s: %s donors median %s\n"%(gene_idx, gene_symbol, rna_tissue_id, len(rna_in_tissue_array), rna_in_tissue_median))
                    
            gene_tissue_median_expression_report_writer.write("%s\n"%"\t".join([str(x) for x in buffer]))
            gene_tissue_present_report_writer.write("%s\n"%"\t".join([str(x) for x in buffer_present]))

        gene_tissue_present_report_writer.close()                    
        gene_tissue_median_expression_report_writer.close()
        
        gene_tissue_median_expression_count_report = "%s.gene_tissue_counts.xls"%self.myArgs.outputRoot
        gene_tissue_median_expression_count_report_writer = open(gene_tissue_median_expression_count_report, "wt")
        
        gene_tissue_median_expression_count_report_writer.write("Tissue\tGenes with TPM>=%s\n"%self.tpm_cutoff)
        for rna_tissue_id in rna_tissue_list:
            gene_tissue_median_expression_count_report_writer.write("%s\t%s\n"%(rna_tissue_id, self.expressed_genes_hash[rna_tissue_id]))
        
        gene_tissue_median_expression_count_report_writer.close()
        
        if (self.DEBUG_PROGRESS):
            sys.stderr.write("[%s] STOP CaptureCORSiV_vs_GeneExpression::reportExpressedGenes\n"%SimpleTime.now())
    ...
    def work(self):
        ...
        self.tpm_cutoff = float(self.myArgs.TPMcutoff)
        sys.stderr.write("TMP cutoff %s\n"%self.tpm_cutoff)
        
        self.loadGTExSampleMap()
        # TODO: keep only tissues with methylation data
        self.loadGTExGeneExpression()
        self.reportExpressedGenes()
        
        # TODO: do only for rna tissues with methylation data
